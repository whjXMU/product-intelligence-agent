...
阶段 03：正式 Agent Workflow 接入已完成并通过总控验收。

## 当前目标

阶段 03 已完成正式 Agent Workflow 接入闭环：版本化 schema、API deterministic runner、`run-workflow` 入口和前端 V1 result/trace 展示已经打通。

- `AnalysisTaskInputV1`、`AnalysisTaskResultV1`、`AgentTraceV1` 已建立；
- API 已新增 `POST /analysis-tasks/:id/run-workflow`；
- 前端任务详情页已支持运行 workflow 并展示 V1 result/trace；
- 阶段 02 mock result/trace 兼容保留；
- `packages/agent-mvp` 仍保持实验资产，不作为正式依赖。

## 当前立即实现

- 进入阶段 04 前的 Agent runner / model provider 架构设计；
- 明确正式 runner 是否从 API deterministic adapter 迁入 `packages/agent-core`；
- 明确 model provider、prompt version、trace、evals 的最小边界。
...
- 2026-06-24：总控复查子窗口 C，web typecheck/build、全局 typecheck/lint/build、API test 和前端入口验证均通过，阶段 03 收口。
...
1. 进入阶段 04 前，先由总控设计正式 Agent runner / model provider 架构；
2. 不急着接真实 LLM，先确定 `packages/agent-core`、provider、prompt、trace、evals 的边界；
